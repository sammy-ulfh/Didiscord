...
class Server:
    ...
    def server(self):
        ...
        msg = ssl.wrap_socket(msg, keyfile="server-key.key", certfile="server-cert.pem", server_side=True)
        msg.listen()

        usrs.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        usrs.bind((self.HOST, self.USERS_PORT))
        usrs = ssl.wrap_socket(usrs, keyfile="server-key.key", certfile="server-cert.pem", server_side=True)
        usrs.listen()

        files.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        files.bind((self.HOST, self.FILES_PORT))
        files = ssl.wrap_socket(files, keyfile="server-key.key", certfile="server-cert.pem", server_side=True)
        files.listen()

        images.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        images.bind((self.HOST, self.IMAGES_PORT))
        images = ssl.wrap_socket(images, keyfile="server-key.key", certfile="server-cert.pem", server_side=True)
        images.listen()

        voice_chat.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        voice_chat.bind((self.HOST, self.VOICE_PORT))
        #voice_chat = ssl.wrap_socket(voice_chat, keyfile="server-key.key", certfile="server-cert.pem", server_side=True)
        voice_chat.listen()
        ...
        voices = ssl.wrap_socket(voices, keyfile="server-key.key", certfile="server-cert.pem", server_side=True)
        voices.listen()
